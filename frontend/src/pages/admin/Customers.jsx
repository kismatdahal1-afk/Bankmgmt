            ...
            {customers.length > 0 ? customers.map(customer => {
              ...
              return (
                <tr key={customer.id}>
                  ...
                  <td>
                    {activeAccount ? (
                      <code style={{ fontFamily: 'monospace', fontSize: '1rem', color: 'var(--accent-color)' }}>{activeAccount.account_number}</code>
                    ) : (
                  ...
                  <td style={{ textAlign: 'right' }}>
                    <div style={{ display: 'inline-flex', gap: '8px' }}>
                      {activeAccount && (
                        <>
                          <Link to={`/admin/transactions/deposit?account_number=${activeAccount.account_number}`} className="btn btn-success btn-sm" style={{ padding: '6px' }} title="Deposit Funds">
                            <span className="material-symbols-rounded" style={{ fontSize: '1.1rem' }}>add_circle</span>
                          </Link>
                          <Link to={`/admin/transactions/withdraw?account_number=${activeAccount.account_number}`} className="btn btn-danger btn-sm" style={{ padding: '6px' }} title="Withdraw Funds">
